...
| **진행 상태 (2026-08-11)** | PortOne **가입 완료** · **통신판매업 신고 진행 중**. 실 API 키·PG사(이니시스 등) 가맹 심사 상태는 미확인 — 사용자 확인 후 연동 착수. |
| 트리거 | 사업자 + PG/에스크로 신청·연동 가능 시점 (= PortOne 라이브 키 확보) |
| 할 일 | 낙찰→**결제 링크** · 웹훅→`paid` · 1시간 미입금 무효 · (가능 시) 2순위 · 인수/자동확정 후 정산 |
| 문서 | `TRUST.md` §0-d · `wakeagain/db.py` deal_* |
| 에이전트 | PG 이야기 시 **결제 링크·웹훅→paid** 만. **pre-PG 우회 기능 새로 만들지 말 것.** |
