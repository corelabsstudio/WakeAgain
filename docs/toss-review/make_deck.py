...
SLIDES = [
    ...
    dict(label="⑥ 결제 경로 — PG 결제창", url="project.html (낙찰 후 · 구매자 화면)",
         img="06c_페이팔결제창.png", layout="side",
         note="PortOne을 통해 실제로 호출된 결제창입니다. "
              "해외 결제(PayPal 채널)는 <b>USD로 청구</b>되므로 환산 금액과 적용 환율을 "
              "구매자에게 먼저 보여줍니다. 국내 카드(토스페이먼츠 채널)는 원화 그대로 청구되며, "
              "채널 승인 완료 후 같은 자리에서 카드 결제창이 열립니다."),
]
